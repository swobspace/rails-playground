...
<3> how much pixel the item should be moved before draggable takes action
<4> don't change the size of our draggable element during the move
...
sortable.on('sortable:stop', function(event) {
  ...
  let url = `${_this.urlValue}/${item_id}`<!--4-->
...
<4> build the item specific url. The url value is set in the same element with `data-controller="sortable"`
